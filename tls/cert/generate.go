...
var (
	host   = flag.String("host", "localhost", "Certificates's comma-seperated host names and IPs")
	certFn = flag.String("cert", "cert.pem", "Certificate file name")
	keyFn  = flag.String("key", "key.pem", "Certificate key file name")
)
...
func main() {
	...
	template := x509.Certificate{
		SerialNumber: serial,
		Subject: pkix.Name{
			Organization: []string{"itsaume mario"},
		},
		NotBefore: notBefore,
